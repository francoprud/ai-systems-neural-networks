...
% betha:
function output = multilayer_perceptron_incremental_momentum(trainingSet, testingSet, needsPlot, trainingPercentage, layersAndSize, minimumError, learningRate, activation_func, activation_func_derived, betha, alpha, adaptativeA, adaptativeB, kEpochs)
  ...
  while (currentError > minimumError)
    ...
    for i = 1:totalInputs
      ...
      for j = (totalEdgesLayers):-1:2
        ...
        deltaWeight = learningRate * [-1 V{j - 1}]' * delta{j} + alpha * previousDeltaWeight{j};
        previousDeltaWeight{j} = deltaWeight;
        networkWeights{j} = networkWeights{j} + deltaWeight;
      end
      deltaWeight = learningRate * [-1 trainingSet{1}(currentIndex, :)]' * delta{1} + alpha * previousDeltaWeight{1};
